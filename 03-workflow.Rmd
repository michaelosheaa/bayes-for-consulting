...
kable(x, caption = "Known value types and corresponding potential distributions")
```
...
d_2013 <- read_sav("C:/Users/trent.henderson/Downloads/2. Australian Election Study, 2013.sav") %>%
  dplyr::select(c(b1, a1, h1, h2)) %>%
  drop_na()

d_2016 <- read_sas("C:/Users/trent.henderson/Downloads/2. Australian Election Study, 2016.sas7bdat") %>%
  dplyr::select(c(B1, A1, H1, H2)) %>%
  drop_na() %>%
  clean_names()
